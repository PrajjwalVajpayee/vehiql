...
const Sidebar = () => {
  ...
  return (
    <>
    <div className='hidden md:flex h-full flex-col overflow-y-auto bg-white shoadow-sm border-r'>
      {routes.map((route)=>{
      ...
      {routes.map((route)=>{
        return <Link key={route.href} href={route.href}
         className={cn("flex flex-col items-center justify-center text-slate-500 text-xs font-medium pl-6 transition-all py-1 flex-1",
            pathName ===route.href ? "text-blue-700 bg-blue-100/50 hover:bg-blue-100 hover:text-blue-700":""
         )}
        >
           <route.icon className='h-5 w-5'/>
